...
export function NotionText({ text, inheritColor }: { text: any, inheritColor?: boolean }) {
    ...
    return text.map((value: any, index: number) => {
        ...
        return (
            ...
            >
                {text.link ? (
                    <a className={`transition-all duration-200 ease-in-out bg-bottom bg-no-repeat bg-no-underline-size hover:bg-underline-size  bg-underline ${inheritColor ? "inherit font-semibold" : text.content.startsWith('@') ? Colors["blue"]?.text.normal : Colors["pink"]?.text.normal}`}
                        after="content-↗"
